...
namespace ComponentPro.Samples
{
    public partial class FileOperation : Form
    {
        ...
        private bool _overwriteIfSizeDiffers;
        private bool _overwriteIfChecksumDiffers;
        ...
        public void Init()
        {
            if (_btns == null)
            {
                ...
                _btns.Add(btnOverwriteDiffSize, TransferConfirmNextActions.OverwriteFilesWithDifferentSizes);
                _btns.Add(btnOverwriteOlder, TransferConfirmNextActions.OverwriteOlderFiles);
                _btns.Add(btnDiffChks, TransferConfirmNextActions.OverwriteFilesWithDifferentChecksums);                
            ...
            _overwriteIfSizeDiffers = false;
            _overwriteIfChecksumDiffers = false;
        ...
        public void Show(Form parent, TransferConfirmEventArgs evt, FileSystem sys)
        {
            ...
            if (evt.ConfirmReason == TransferConfirmReason.FileAlreadyExists)
            {
                ...
                if (_overwriteIfSizeDiffers)
                {
                    evt.NextAction = TransferConfirmNextActions.OverwriteFilesWithDifferentSizes;
                    return;
                }

                if (_overwriteIfChecksumDiffers)
                {
                    evt.NextAction = TransferConfirmNextActions.OverwriteFilesWithDifferentChecksums;
                    return;
                }

                if (_overwriteOlder)
                {
                    evt.NextAction = TransferConfirmNextActions.OverwriteOlderFiles;
                    return;
                }
                ...
                string itemType = evt.SourceFileInfo.IsFile ? "file" : "directory";

                // format the text according to TransferState (Downloading or Uploading)
                const string messageFormat = "Are you sure you want to overwrite {6}: {0}\r\n{1} Bytes, {2}\r\n\r\nWith {6}: {3}\r\n{4} Bytes, {5}";

                lblMessage.Text = string.Format(messageFormat,
                                                evt.DestinationFileInfo.FullName, evt.DestinationFileInfo.Length == -1 ? 0 : evt.DestinationFileInfo.Length,
                                                evt.DestinationFileInfo.LastWriteTime,
                                                evt.SourceFileInfo.FullName, evt.SourceFileInfo.Length == -1 ? 0 : evt.SourceFileInfo.Length,
                                                evt.SourceFileInfo.LastWriteTime, itemType);

                Text = (evt.SourceFileInfo.IsFile ? "File" : "Directory") + " already exists";
            }
            else
            {
                if (evt.Exception != null)
                {
                    ...
                    {
                        System.Diagnostics.Trace.WriteLine("NullReferenceException detected.");
                    }
        ...
        {
            _args.NextAction = TransferConfirmNextActions.OverwriteOlderFiles;
        ...
        {
            _args.NextAction = TransferConfirmNextActions.OverwriteFilesWithDifferentSizes;
            _overwriteIfSizeDiffers = true;
            _args = null; // Release mem
            Close();
        }

        private void btnOverwriteDiffChecksum_Click(object sender, System.EventArgs e)
        {
            _args.NextAction = TransferConfirmNextActions.OverwriteFilesWithDifferentChecksums;
            _overwriteIfChecksumDiffers = true;
            _args = null; // Release mem
            Close();
        }
